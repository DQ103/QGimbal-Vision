...
MODEL_ZOO_DIR="${1:-$HOME/awnpu_model_zoo}"
YOLO_DIR="$MODEL_ZOO_DIR/examples/yolov8"
...
if [[ ! -d "$YOLO_DIR" || ! -d "$NPU_RT_DIR" || ! -d "$NPU_LIB_DIR" ]]; then
  ...
  echo "Expected examples/yolov8 and common/npuruntime/lib_linux_aarch64/A733." >&2
  exit 1
fi
...
g++ -std=c++11 -O2 \
  ...
  ../main.cpp ../yolov8_6_pre.cpp ../yolov8_6_post.cpp \
...
echo "Built: $BUILD_DIR/yolov8_demo_a733"
echo "Runtime:"
