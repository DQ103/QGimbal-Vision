...
exec "$ROOT_DIR/scripts/run_a7a_imx415_competition_web.sh" \
  ...
  --aim-enter-radius-ratio "${AIM_ENTER_RADIUS_RATIO:-0.146}" \
  --aim-exit-radius-ratio "${AIM_EXIT_RADIUS_RATIO:-0.165}" \
  --aim-confirm-frames "${AIM_CONFIRM_FRAMES:-1}" \
  --laser-hold-frames "${LASER_HOLD_FRAMES:-2}" \
  --laser-min-luma "${LASER_MIN_LUMA:-165}" \
  --laser-fallback-min-luma "${LASER_FALLBACK_MIN_LUMA:-165}" \
